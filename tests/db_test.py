...
def test_new_database():
    ...
    res = db.execute("SELECT Name, Url FROM Service").fetchone()
    assert res['Name'] == 'Netflix'

def test_from_backup(db_file=test_utils.GOOD_DB_FILE, _salt_file=test_utils.GOOD_SALT_FILE):
    db = SqliteDatabase.from_backup(db_file, _salt_file, test_utils.TEST_PASSWORD)
    assert db
...
def test_backup():
    (db, salt) = SqliteDatabase.from_script(SqlStatements.CREATE_TABLES, test_utils.TEST_PASSWORD)
    
    with open(test_utils.TEST_SALT_FILE, 'wb') as f:
        f.write(salt)
    
    db.backup(test_utils.TEST_DB_FILE)
    test_from_backup(test_utils.TEST_DB_FILE, test_utils.TEST_SALT_FILE)
    
def test_from_backup_corrupted_db():
    db = SqliteDatabase.from_backup(test_utils.BAD_DB_FILE, test_utils.GOOD_SALT_FILE, test_utils.TEST_PASSWORD)
    assert not db
    
def test_from_backup_bad_salt():
    db = SqliteDatabase.from_backup(test_utils.GOOD_DB_FILE, test_utils.BAD_SALT_FILE, test_utils.TEST_PASSWORD)
    assert not db